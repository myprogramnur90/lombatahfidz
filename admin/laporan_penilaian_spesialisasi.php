...
$query_laporan = "
    SELECT 
        ...
        MAX(CASE WHEN j.spesialisasi = 'Makhraj' THEN pen.skor_spesialisasi END) as skor_makhraj,
        MAX(CASE WHEN j.spesialisasi = 'Kelancaran Hafalan' THEN pen.skor_spesialisasi END) as skor_fluency,
        MAX(CASE WHEN j.spesialisasi = 'Tajwid dan Adab' THEN pen.skor_spesialisasi END) as skor_tajwid_adab,
    ...
    ORDER BY skor_rata_rata DESC, p.nama_lengkap
";
                                        ...
                                        <?php if (count($laporan_list) > 0): ?>
                                            <?php $rank = 1; foreach ($laporan_list as $laporan): ?>
                                                <tr>
                                                    ...
                                                    <td class="skor-cell">
                                                        <?php if ($laporan['skor_rata_rata']): ?>
                                                            <span class="<?php echo $laporan['skor_rata_rata'] >= 80 ? 'skor-excellent' : ($laporan['skor_rata_rata'] >= 60 ? 'skor-good' : 'skor-poor'); ?>">
                                                                <strong><?php echo number_format($laporan['skor_rata_rata'], 1); ?></strong>
                                                            </span>
                                                        <?php else: ?>
                                                            <span class="text-muted">-</span>
                                                        <?php endif; ?>
