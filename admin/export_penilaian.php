...
// Data
$rank = 0; // peringkat yang ditampilkan
$position = 0; // posisi iterasi (1,2,3,...)
$last_score = null; // skor_terbobot terakhir untuk handle tie
foreach ($data as $row) {
    $status = '';
    ...
    // Hitung peringkat berdasarkan skor_terbobot (tie mendapat peringkat sama)
    $position++;
    $current_score = isset($row["skor_terbobot"]) ? (float)$row["skor_terbobot"] : 0.0;
    if ($last_score === null || $current_score !== $last_score) {
        $rank = $position;
    ...
    $csv_row = [
        ...
        $row['skor_terbobot'] ? number_format($row['skor_terbobot'], 1) : '-',
